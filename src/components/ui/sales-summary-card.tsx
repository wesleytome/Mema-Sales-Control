...
export function SalesSummaryCard({ sales, installments, payments, isLoading }: SalesSummaryCardProps) {
  ...
  const monthlyMetrics = useMemo<MonthlyMetricData[]>(() => {
    ...
    filteredInstallments.forEach((inst) => {
      ...
      if (remaining > 0) {
        buckets[idx].toReceive += remaining;
      }
  ...
  const chartSeries = useMemo(() => {
    return monthlyMetrics.map((monthData) => ({
            ...
            return (
              ...
              >
                <div className="flex items-center justify-between gap-2 mb-2">
                  <span className={cn('text-xs', isActive ? 'text-foreground/80' : 'text-muted-foreground')}>{card.title}</span>
                  <span className={cn('flex h-7 w-7 items-center justify-center rounded-lg', isActive ? card.iconClass : 'text-muted-foreground bg-muted')}>
            ...
            <ChartContainer config={chartConfig} className="h-[220px] w-full">
              <LineChart accessibilityLayer data={chartSeries} margin={{ top: 16, right: 8, left: 8, bottom: 0 }}>
                ...
                <YAxis
                  ...
                  tick={{ fontSize: 11 }}
                  tickFormatter={(value) => {
                    if (activeMetric === 'salesCount') return `${value}`;
